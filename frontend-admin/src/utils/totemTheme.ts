...
export function hasAttractMedia(blocks: Array<{ type: string; visible: boolean; imageUrl?: string; videoUrl?: string }>): boolean {
  return blocks.some(block => block.type === 'video' && block.visible && Boolean(block.videoUrl))
    || blocks.some(block => block.type === 'hero' && block.visible && Boolean(block.imageUrl))
}
...
export function hexToRgbString(hex: string): string | null {
  ...
  return rgb ? rgbToString(rgb) : null
}
